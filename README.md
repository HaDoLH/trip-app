...
- 📸 **截圖丟進來取字**：手機上用系統內建的取字功能（iOS 實況文字 / Android Google 鏡頭），一鍵貼上解析
- 🖥 電腦上則把截圖丟給 Claude Code 辨識，產出的資料從「匯入資料」貼進來
- ✅ 加入前會先出確認卡，逐筆勾選、可修改，不會亂寫進去
- 🔍 **店名待確認**：只抓到 IG 帳號或地址時會標出來，一鍵用地址開 Google 地圖或直接打開那個 IG 帳號；切回 App 時編輯視窗會自動打開等你貼上店名
  （沒做成全自動：免費的 OpenStreetMap 測過，連火星 Cafe Mars 都查不到；Google 地點 API 要綁信用卡）
- ⠿ **拖曳排序**：按住每一列左邊的握把上下拖
...
「新增」分頁按 🖼 選一張截圖（也可以直接貼上、或在電腦拖曳進來），App 會把圖放大顯示，接著：

- **iPhone**：在圖上長按 →「拷貝文字」→ 按「貼上並解析」
- **Android**：用 Google 鏡頭取字 → 複製 → 按「貼上並解析」

關鍵是 **iOS 的實況文字對網頁裡顯示的圖片一樣有效**，所以不用切出 App。
截圖只放在記憶體裡（`URL.createObjectURL`），不會存進 localStorage —— 存圖片很快就會把 5MB 空間塞爆。

### 電腦（交給 Claude Code）

瀏覽器沒有內建取字，所以走匯入路線，格式如下。
